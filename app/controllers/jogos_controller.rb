class JogosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @jogo.update(jogo_params)
        format.html { redirect_to @jogo, notice: "Jogo was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to jogos_path, notice: "Jogo was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def jogo_params
      params.expect(jogo: [ :title, :description, :developer, :genre, :price ])
    end
end
